...
const kwonumoskSource = { label: 'Vermont Fish & Wildlife Otter Creek–Kwonumosk official GIS feature', url: 'https://anrmaps.vermont.gov/arcgis/rest/services/Open_Data/OPENDATA_ANR_TOURISM_SP_NOCACHE_v2/MapServer/163/query?where=AccessName%20LIKE%20%27%25Kwonumosk%25%27&outFields=*&outSR=4326&returnGeometry=true&f=json', provider: 'local' as const };
...
function makeRoute(spec: { id: string; start: OtterPoint; end: OtterPoint; miles: number; summary: string; note: string }): River {
  return buildStarterPlanningRoute({
    ...
    guide: paddleMap, sources: [accessGuide, kwonumoskSource, basinPlan, paddleGuide, waterQuality], coordinateNote: 'Coordinates are access-area anchors for named state, municipal and paddling-map access locations; they are not surveyed water-entry edges. Verify the carry, dam boundary and current parking before departure.', coordinateSourceUrl: accessGuide.url, reviewDate: '2026-09-16',
  });
}
...
export const vermontOtterCreekRoutes: River[] = [
  ...
  makeRoute({ id: 'otter-creek-quaker-village-weybridge', start: quaker, end: weybridge, miles: 5, summary: 'A Middlebury-to-Weybridge reach through the lower Middlebury corridor with the Weybridge State Fishing Access take-out.', note: 'Use the Vermont Fish & Wildlife Otter Creek–Kwonumosk access-area anchor at 44.086,-73.2472. Its state GIS point is about 2 miles from the former Weybridge coordinate and does not identify the exact steps/water edge; confirm the access path, parking, dam boundary, and portage on site. The corrected endpoint requires regenerated route geometry before publication.' }),
  makeRoute({ id: 'otter-creek-weybridge-vergennes', start: weybridge, end: vergennes, miles: 7, summary: 'A popular lower Otter Creek flatwater reach from Weybridge State Fishing Access to the Vergennes town dock below the falls.', note: 'The Vermont Fish & Wildlife state GIS identifies Kwonumosk as an Otter Creek boating/fishing access in Weybridge with step access. The stored coordinate is the facility anchor, not a surveyed water edge; verify the carry and dam boundary. Treat Vergennes falls as a hard boundary and confirm downstream town-dock conditions.' }),
  ...
  makeRoute({ id: 'otter-creek-weybridge-fort-cassin', start: weybridge, end: fortCassin, miles: 15, summary: 'A staged lower Otter Creek itinerary from Weybridge through Vergennes to Fort Cassin at the Lake Champlain end of the corridor.', note: 'Use the documented Kwonumosk and Fort Cassin state access points, with Vergennes as an intermediate bailout. Portage the Kwonumosk and Vergennes dam/falls boundaries; the Kwonumosk endpoint correction is held pending route geometry regeneration.' }),
];
