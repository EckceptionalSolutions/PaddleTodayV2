...
type FunnelRow = { state: string; hubViews: number; trips: number; routeOpens: number; tripConversion: string; routeConversion: string };
...
async function main() {
  ...
  const rows = new Map<string, { hubViews: Set<string>; trips: Set<string>; routeOpens: Set<string> }>();
  for (const [index, event] of events.entries()) {
    const name = event.event || event.name || event.event_name;
    if (name !== 'river_hub_viewed' && name !== 'corridor_selected' && name !== 'corridor_trip_selected' && name !== 'route_opened' && name !== 'Route view') continue;
    ...
    const bucket = rows.get(state) ?? { hubViews: new Set(), trips: new Set(), routeOpens: new Set() };
    const identity = String(event.session_id || event.user_id || event.user_pseudo_id || props.session_id || props.user_id || index);
    if (name === 'river_hub_viewed' || name === 'corridor_selected') bucket.hubViews.add(`${corridor}:${identity}`);
  ...
  const reportRows: FunnelRow[] = [...pilotStates].sort().map((state) => {
    const value = rows.get(state) ?? { hubViews: new Set<string>(), trips: new Set<string>(), routeOpens: new Set<string>() };
    return {
      state,
      hubViews: value.hubViews.size,
      trips: value.trips.size,
      routeOpens: value.routeOpens.size,
      tripConversion: value.hubViews.size ? `${Math.round(value.trips.size / value.hubViews.size * 100)}%` : '-',
      routeConversion: value.hubViews.size ? `${Math.round(value.routeOpens.size / value.hubViews.size * 100)}%` : '-',
    };
  });
  const lines = [
    ...
    '| State | River-hub views | Trip selections | Route-detail opens | Trip rate | Route-open rate |', '| --- | ---: | ---: | ---: | ---: | ---: |',
    ...reportRows.map((row) => `| ${row.state} | ${row.hubViews} | ${row.trips} | ${row.routeOpens} | ${row.tripConversion} | ${row.routeConversion} |`),
