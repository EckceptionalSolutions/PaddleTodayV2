...
const targetPerState = 40;
const report = {
  version: 1,
  generatedAt: new Date().toISOString(),
  targetPerState,
  selectionPolicy: 'demand-first, scored-first',
  notes: 'Counts include current route inventory. Stop below 40 only after a documented research shortfall; never pad with unsupported or redundant reaches.',
...
await writeFile('docs/operations/lower-48-expansion-40-progress.json', `${JSON.stringify(report, null, 2)}\n`);
console.log(`Wrote ${report.states.length}-state 40-route expansion report.`);
