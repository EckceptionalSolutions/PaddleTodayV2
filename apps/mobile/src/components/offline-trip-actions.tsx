...
import { downloadOfflineTrip, listOfflineTrips, loadOfflineTrip, mappedAccess, offlineTripBytes, offlineTripId, removeOfflineTrip, retryOfflineGeometry, type OfflineTrip } from '../lib/offline-trips';
import type { TripDraft, TripDraftTarget } from '../lib/trip-drafts';
...
export function PrepareOfflineTrip({ detail, putIn, takeOut, draft, ready, saveDraft, registerCancellation }: {
  ...
  return <SectionCard title="Prepare for offline use" subtitle="Save landings, reference geometry, logistics, timing and notes on this device. Background maps and current conditions are not included.">
    {packet ? <OfflinePacketCard packet={packet} disabled={busy || loading} onRemoved={() => setPacket(null)} onUpdated={setPacket} /> : null}
    {message ? <Text accessibilityLiveRegion="polite" style={styles.body}>{message}</Text> : null}
    {loading ? <Text style={styles.body}>Checking offline trip…</Text> : <AppButton label={packet ? 'Update offline trip' : 'Download offline trip'} busy={busy} busyLabel="Downloading offline trip…"
    ...
    {busy ? <AppButton label="Cancel offline download" variant="secondary" onPress={() => request.current?.abort()} /> : null}
    {message && !busy ? <AppButton label="Retry offline trip" variant="secondary" onPress={() => void reload()} /> : null}
  </SectionCard>;
}

function OfflinePacketCard({ packet, disabled = false, onRemoved, onUpdated }: {
  packet: OfflineTrip; disabled?: boolean; onRemoved: () => void; onUpdated: (packet: OfflineTrip) => void;
}) {
  ...
  return <View style={styles.card}>
    ...
    {message ? <Text accessibilityLiveRegion="polite" style={styles.body}>{message}</Text> : null}
    {open ? <OfflineTripView packet={packet} onClose={() => setOpen(false)} /> : null}
  </View>;
}

export function SavedOfflineTrips() {
  const [records, setRecords] = useState<OfflineTrip[]>([]), [error, setError] = useState(''), [loading, setLoading] = useState(true);
  const epoch = useRef(0);
  const refresh = useCallback(async () => {
    const generation = ++epoch.current; setLoading(true);
    try { const result = await listOfflineTrips(AsyncStorage);
      if (epoch.current === generation) { setRecords(result.records); setError(result.unreadable ? 'Some offline trips could not be read. Nothing has been removed.' : ''); } }
  ...
  return <SectionCard title="Offline trips" subtitle="Open downloaded trip plans without a connection. Current conditions are unavailable in saved copies.">
    ...
    {records.map(packet => <OfflinePacketCard key={offlineTripId(packet.target)} packet={packet} disabled={loading}
