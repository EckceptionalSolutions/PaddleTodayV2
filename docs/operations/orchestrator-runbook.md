...
`routeReadiness` is the route-development label and must not be confused with the durable review disposition. Blocked gauges are subdivided into `candidate`, `research_needed`, `existing_route_gap`, or `deferred`. `candidate` means structural route-family or direct-corridor evidence already exists; `research_needed` means route discovery remains incomplete; `existing_route_gap` means an existing route or equivalent provider already represents the reach; and `deferred` means an explicit retry condition or materially new evidence is required. Covered, screened-out, stale, and unreviewed gauges map to `published`, `screened_out`, `unsupported`, and `unreviewed` respectively. Text inference must never move a gauge to `screened_out`.
