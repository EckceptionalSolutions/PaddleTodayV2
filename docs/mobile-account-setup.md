...
This is an operational checklist for the account API and native authentication foundation. The mobile sign-in screen and device-triggered backup/sync are implemented. Production storage access and Firebase Admin credentials are configured, and the account API is deployed. Keep customer sign-up gated until provider setup and physical-device release checks are complete.
...
Production runs on Azure App Service `paddletodayapi`. The private `account-data` container is in the existing `paddletoday` storage account in Central US. `ACCOUNT_DATA_CONTAINER_SAS_URL` is configured in App Service with HTTPS-only access and read/create/write/delete permissions; its current SAS expires at **2026-12-22 11:29:59 UTC**. Rotate it before expiry. `FIREBASE_SERVICE_ACCOUNT_JSON` is configured; its project ID was checked against `paddletoday-9933a`, and the App Service is running. On 2026-09-24, an unauthenticated `GET https://paddletoday.com/api/account` returned HTTP 401 `authentication_required`, confirming the account route is deployed and rejects requests without a Firebase ID token. In Azure Portal, open **App Services > paddletodayapi > Settings > Environment variables > App settings** to manage backend settings.
