...
- Added `new-river-nc88-wagoner`, a distinct five-mile planning section from the NCWRC-listed N.C. 88 Bridge public canoe access to New River State Park Wagoner Access. The current park map supplies the section mileage and paddle-time estimate; NCWRC supplies the bridge access coordinate; USGS `03161000` remains proxy context only, so no numeric cutoff is transferred.
