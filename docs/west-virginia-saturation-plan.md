...
The current catalog has 22 West Virginia route cards:
...
The research ledger has decisions for 24 priority gauges and 26 bounded route candidates across 7 corridors. The next saturation step is therefore not to add arbitrary mileage; it is to turn the remaining route-capable gauges into public-access pairs with an auditable threshold, a safety profile, and complete logistics.
...
7. Coal mainstem: Meadowood Park to Lower Falls, only after the portage boundary is mapped.
...
The next concrete batch is **Wave 2 B-5 or the lower Coal C-2**, selected after confirming which remaining Coal River access pair has the strongest current parking and threshold evidence. Keep the Upper Falls portage boundary explicit for any mainstem card. Keep Wave 1B (Sandy Beach to Clifton Ford) gated until the informal Sandy Beach endpoint is proven distinct and publicly usable; do not implement Wave 1D until the intended Clendenin access is verified and not the closed Mink Shoals site.
